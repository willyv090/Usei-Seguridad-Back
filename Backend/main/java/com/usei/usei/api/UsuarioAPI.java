...
public class UsuarioAPI {
   ...
   @PostMapping
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequestUserDTO loginRequestUser) {
        try {
            // Usa SecurityBL para validar y manejar intentos/expiración
            ...
            switch (status) {
                case OK: {
                    ...
                    if (usuario.isPresent()) {
                        ...
                        if (Boolean.TRUE.equals(user.getCambioContrasenia())) {
                            ...
                            payload.put("id_usuario", user.getIdUsuario());
                            payload.put("correo", user.getCorreo());
                            payload.put("carrera", user.getCarrera());
                            payload.put("nombre", user.getNombre());
                            payload.put("ci", user.getCi());
                            payload.put("cambio_contrasenia", user.getCambioContrasenia());
                    ...
                    return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                            .body("Usuario no encontrado tras validación");
                }
                case BLOQUEADO: {
                ...
                case CREDENCIALES: {
                    // Intentos restantes (si hay Contrasenia vinculada)
                    Optional<Usuario> u = usuarioService.findByCorreo(loginRequestUser.getCorreo());
                    Integer remaining = null;
                    if (u.isPresent() && u.get().getContraseniaEntity() != null) {
                        remaining = u.get().getContraseniaEntity().getIntentosRestantes();
                    }
                    Map<String,Object> payload = new HashMap<>();
                ...
                // ⚠️ Mantengo tu case existente (si tu enum lo incluye). Si no existe en tu enum, comenta este bloque.
                ...
                default:
                    return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                            .body(new UnsuccessfulResponse("401", "Credenciales incorrectas", "/usuario/login"));
            }
        } catch (Exception e) {
